...
class GroupBoxCard {
  ...
  static render(groupBox, options = {}) {
    const { showNewBadge = true, showDebugInfo = false } = options;
    ...
    return `
            <div class="${cardClasses}" data-action="open-group-box" data-id="${
      groupBox.groupBoxId
    }">
                ${
    ...
    if (groupBox.isOrganizerOnly) {
      ...
      const triesLeft = `Tries Left: ${
        groupBox.userRemainingTries !== undefined
          ...
          : groupBox.settings?.triesPerPerson || 0
      }`;
      return `
                <div class="lootbox-stats">
  ...
  static renderDebugInfo(groupBox) {
    return `
            <div class="group-box-debug-info" style="font-size: 10px; color: #666; padding: 4px; background: #f0f0f0; margin-top: 4px;">
                ID: ${groupBox.groupBoxId} | Creator: ${
      groupBox.isCreator ? "Yes" : "No"
    } | Organizer: ${groupBox.isOrganizerOnly ? "Yes" : "No"}
            </div>
        `;
  }
  ...
  static renderWithExpiration(groupBox, options = {}) {
    ...
    const warningBadge = `
            <div class="expiration-warning-badge ${
              expirationInfo.isExpired ? "expired" : "expiring"
            }">
                <span class="expiration-warning-label">${
   ...
   */
  static renderError(
    errorMessage = "Invalid group box data",
    groupBoxId = null
  ) {
    return `
  ...
  static validateGroupBox(groupBox) {
    ...
    if (groupBox.lootboxData) {
      if (
        !Array.isArray(groupBox.lootboxData.items) ||
        groupBox.lootboxData.items.length === 0
      ) {
